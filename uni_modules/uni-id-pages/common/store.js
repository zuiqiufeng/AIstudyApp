...
function hasValidLoginState(userInfo = {}) {
	const {
		tokenExpired = 0
	} = uniCloud.getCurrentUserInfo()

	return Object.keys(userInfo || {}).length != 0 || tokenExpired > Date.now()
}
const data = {
	userInfo: hostUserInfo,
	hasLogin: hasValidLoginState(hostUserInfo)
}
...
export const mutations = {
	...
	async updateUserInfo(data = false) {
		if (data) {
			...
			try {
				...
				try {
					nextUserInfo.realNameAuth = await uniIdCo.getRealNameInfo()
				} catch (e) {
					console.error('getRealNameInfo failed', e.message, e.errCode);
				}
	...
	async setUserInfo(data, {cover}={cover:false}) {
		...
		store.hasLogin = hasValidLoginState(store.userInfo)
