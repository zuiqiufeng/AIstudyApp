...
<script>
	...
	export default {
		...
		methods: {
			...
			showLoginLoading() {
				if (this.loginLoadingVisible) {
					return
				}
				this.loginLoadingVisible = true
				uni.showLoading({
					mask: true
				})
			},
			...
			async login_before(type, navigateBack = true, options = {}) {
				...
				// #ifdef APP
				loginOptions.univerifyStyle = this.univerifyStyle
				if (type === 'weixin') {
			...
			login(params, type) { //联网验证登录
				if (type === 'weixin' && (!params || !params.code)) {
					this.hideLoginLoading()
					return uni.showToast({
						title: '微信登录失败，请重试',
						icon: 'none',
				...
				uniIdCo[action](params).then(result => {
					if (result.newToken?.token && result.newToken?.tokenExpired) {
